...
excerpt: >
    ...
    A method built to speed up image generators can be carried over, almost unchanged, to models that generate proteins and molecules. The reason it transfers is simple: the step size is just a number with no direction in space, so it can be fed in through a side channel without ever touching the 3D geometry the model works with.
...
This document shows how a method built to speed up image generators can be carried over to models that generate proteins and molecules. The original method works like this: take a trained flow-matching model, freeze it, and add a small new input called the step size, which tells the model how big a jump to take. By training only this small part, the model learns to produce good results in just a few steps instead of many. The key reason this transfers is simple: the step size is just a number with no direction in space, so it can be fed in through a side channel without ever touching the 3D geometry the model works with. In protein and molecule models, which are carefully built so that rotating the input rotates the output, this means the method can teach the model to take big jumps without disturbing the physics it has already learned. We point to the specific models this applies to (FrameFlow, FoldFlow, and Proteina for protein backbones; EquiFM, MolFlow, and FlowDock for molecules and docking), the datasets that take the place of ImageNet and LAION-POP, the metrics that replace FID, and the one real change the new setting requires: following curved paths on the space of rotations instead of straight lines.
...
At inference one integrates the learned ODE, which typically requires tens to hundreds of function evaluations (NFE). The shortcut formulation of Frans et al. (2024) extends this by additionally conditioning the network on a desired step size $$d$$, giving $$s_\theta(x_t, t, d)$$, so that a single model can take jumps of size $$d$$ and thereby generate under any inference budget, from many steps down to a single forward pass.

The original shortcut model trains this $$d$$-conditioned network from scratch. The method considered here instead *post-trains* an already-converged FM checkpoint: it injects the $$d$$ pathway into the frozen model and trains only that pathway plus the conditioning projections, preserving the expensive pretraining investment while unlocking few-step sampling at a small additional cost. On DiT-S/8 over ImageNet this trains roughly one third of the parameters; on Flux.1-Dev (12B parameters) it turns a 32-step text-to-image model into a competitive few-step sampler in a short post-training run.
...
The self-consistency target is produced by an exponential-moving-average (EMA) teacher under stop-gradient, which damps the oscillations that a self-referential objective would otherwise induce. Only the $$d$$ embedder and the $$W_{\text{ada}}$$ projections receive gradients; everything else is frozen.

## 2. Why the field is the right place to apply this
...
The landscape of target models is broad and already mature. FrameFlow adapts the FrameDiff diffusion model to flow matching and reports roughly five times fewer sampling steps at better designability; FoldFlow builds SE(3) (and stochastic SE(3)) flow matching with optimal-transport couplings; Proteina scales flow-based backbone generation to large models and datasets; and P2DFlow applies SE(3) flow matching to conformational ensembles. On the small-molecule side, equivariant flow-matching models such as EquiFM and MolFlow generate 3D conformers, FlowDock learns a geometric flow over ligand poses for blind docking, and discrete flow models extend the same framework to coupled atom-type and coordinate generation. Materials-structure analogues (MolCrystalFlow, MOFFlow) use the identical machinery.

The practical bottleneck across all of these is sampling speed. Diffusion-based and flow-based structural generators typically require hundreds of iterative steps, and de novo design needs thousands to millions of candidate structures to find a few that fold and function. This makes inference cost the binding constraint, precisely the constraint the shortcut post-training method was built to relax. The problem is recognised in the field: *Distilled Protein Backbone Generation* (Xie et al., 2025) adapts score-identity distillation to protein backbones and reports more than a twenty-fold sampling speedup at comparable designability to a Proteina teacher, while noting that a naive transfer of image distillation methods produces unacceptably low designability and that careful, domain-aware adaptation is required. That paper validates both the demand for few-step structural generators and the need for an adaptation that respects the domain, which is what the present method offers from a different, post-training-based angle.

## 3. The portability argument

The reason the recipe transfers is not superficial similarity of architectures; it is a property of what $$d$$ is. Conditioning signals in an SE(3)-equivariant network fall into two categories. The first is geometric, equivariant data: the residue frames, atomic coordinates, and pair representations. When the input is rotated or translated, these quantities transform accordingly, and the network's update rules (through invariant point attention, tensor-product layers, or equivariant message passing) are constructed to respect that transformation. The second category is scalar invariants: the diffusion or flow time $$t$$, an optional fold-class or property label, and, in the transferred method, the step size $$d$$. These carry no geometric type; they are unchanged under any rotation or translation of the structure.

A scalar invariant can only be injected through invariant channels. In a frame-based network this is the adaLN-style modulation of the invariant feature track, or an additive bias inside invariant point attention; in an equivariant message-passing network it is the modulation of the scalar (type-0) features. Because $$d$$ enters only these invariant pathways and never alters the equivariant update rules, adding the $$d$$ pathway cannot break the network's equivariance. This is the molecular analogue of the clean $$\mathbf{c}$$ versus $$\mathbf{x}$$ boundary in the image model: there, $$\mathbf{c}$$ touched only $$W_{\text{ada}}$$ and never the data stream; here, $$d$$ touches only the invariant conditioning and never the equivariant geometry. The portability of the method is therefore a direct consequence of $$d$$ being a scalar, not a fortunate empirical coincidence.

## 4. What stays the same and what changes

### 4.1 Carried over unchanged

The two-term loss, the EMA teacher under stop-gradient, the freezing of the backbone with training restricted to the conditioning pathway, the zero-initialisation-at-start that yields identity, and the sinusoidal-plus-MLP embedding of the scalar $$d$$ all carry over verbatim. These are the parts of the method that depend only on the existence of a conditioning vector and a frozen teacher, both of which the target models have.
...
**Data.** The training distribution plays the role ImageNet and LAION-POP played in the image work. The standard choice is PDB monomers, length-filtered (for example 60 to 256 residues) and clustered at 30 to 40% sequence identity to remove redundancy, the same split FrameFlow and FoldFlow train on. SCOPe or CATH fold labels provide the optional class conditioner that the ImageNet class label provided. For scaling, an AlphaFold-DB distillation set mirrors how Proteina scales up. As in the Flux pipeline, frames and conditioner embeddings are pre-cached so that no auxiliary encoder runs during post-training.

**Evaluation.** FID is meaningless for structures and is replaced by a self-consistency pipeline. *Designability* is the fraction of generated backbones with self-consistency RMSD below 2 angstrom: each generated backbone is passed to ProteinMPNN to design a sequence, the sequence is folded by ESMFold, and the refolded structure is compared by RMSD to the generated backbone. *Diversity* counts structural clusters (via MaxCluster or Foldseek) among designable samples, and *novelty* is the maximum TM-score against the PDB, with lower meaning more novel. The headline study mirrors the image work exactly: plot designability, diversity, and novelty against NFE, and against post-training steps, and show that designability is held at the teacher's level while the step count collapses from roughly one hundred or more down to a handful, training under forty percent of the parameters.

## 6. Concrete target 2: small molecules, conformers, and docking

The same recipe applies to molecular generation with different data and metrics. For 3D conformer generation, the teacher is an equivariant FM model in the EquiFM or MolFlow family, trained on GEOM-Drugs and QM9; the relevant metrics are coverage and matching RMSD against reference ensembles, together with chemical validity and strain energy, each reported against NFE. For structure-based drug design, the teacher generates ligands inside a protein pocket (as in the MolFORM family), trained on CrossDocked2020 or PDBbind, with the pocket embedding serving as the cached conditioner; the metrics are docking score (for example AutoDock Vina), drug-likeness (QED), synthetic accessibility, and pose RMSD. Docking itself is a high-value target: FlowDock's geometric flow over ligand poses, reduced to a few steps, would raise virtual-screening throughput substantially, since screening is dominated by the per-pose sampling cost.
